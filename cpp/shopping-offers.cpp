...
public:
    void dfs(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {
        ...
        for (int i=0; i<special.size(); i++) {
            ...
            if (j==needs.size()) {
                useSpecial=true;
                totalPrice+=special[i][special[i].size()-1];                
                for (int k=0; k<needs.size(); k++) needs[k]-=special[i][k];
        ...
        if (!useSpecial) {
            ...
            minPrice=min(minPrice, totalPrice+sum);            
        }
    }
    ...
    int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {        
        ...
        totalPrice=0;
        int origPrice=0;
        for (int i=0; i<needs.size(); i++) origPrice+=price[i]*needs[i];
        dfs(price, special, needs);
        
        return min(minPrice, origPrice);
    }
private:
    int minPrice;
    int totalPrice;    
