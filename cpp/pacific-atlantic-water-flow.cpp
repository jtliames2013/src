...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& matrix) {
        ...
        vector<vector<int>> touch(m, vector<int>(n));
        for (int i=0; i<m; ++i) {
...
private:
    void dfs(vector<vector<int>>& matrix, vector<vector<int>>& touch, int row, int col, int m, int n, int val) {
        touch[row][col]|=val;
        for (int k=0; k<4; k++) {
            int nr=row+delta[k][0];
            int nc=col+delta[k][1];
            if (nr>=0 && nr<m && nc>=0 && nc<n && (touch[nr][nc] & val)==0 && matrix[row][col]<=matrix[nr][nc]) {
                dfs(matrix, touch, nr, nc, m, n, val);
            }
        }
    }
