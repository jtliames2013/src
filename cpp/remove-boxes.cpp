546. Remove Boxes Add to List
DescriptionHintsSubmissionsSolutions
Total Accepted: 1540
Total Submissions: 5198
Difficulty: Hard
Contributors:
ckcz123
...
class Solution {
public:
    int dfs(vector<int>& boxes, int l, int r, int k, int dp[100][100][100]) {
        if (l>r) return 0;
        ...
        for (int i=l+1; i<=r; i++) {
            if (boxes[i]==boxes[l]) {
                res=max(res, dfs(boxes, l+1, i-1, 0, dp)+dfs(boxes, i, r, k+1, dp));
            }
        }
        dp[l][r][k]=res;
        return res;
